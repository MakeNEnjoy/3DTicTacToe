use crate::{tic_simulator::Strategy, tic::*, minmax::*, game};
...
fn get_move_player1(steps_to_search: usize, game_state: &Board) -> Option<Board> {
    ...
    for state in states {
        let score = max_score::<Board, AlmostWinHeuristic>(&state, steps_to_search);
        scores.push(score);
...
fn get_move_player2(steps_to_search: usize, game_state: &Board) -> Option<Board> {
    ...
    let mut scores = Vec::new();
    for state in states {
        let score = min_score::<Board, AlmostWinHeuristic>(&state, steps_to_search);
        scores.push(score);
        if let Some(bs) = best_score {
            if bs < score {
                best_score = Some(score);
